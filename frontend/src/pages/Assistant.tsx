        ...
        <div className="row" style={{ marginTop: 10 }}>
          ...
          {/* 0건은 LLM 설명을 안 거친 확정 응답이다.
              **`llm_calls` 로는 그걸 판정할 수 없다.** 예전 판본은 `> 1` 이면
              ...
              그 판정이다).
              **설명 호출이 되살아나는 것을 막는 것은 검사가 아니라 시그니처다** —
              `_no_results(filters)` 는 LLM 클라이언트를 받지 않으므로 부를 수가
              없다(ADR-0036 이 `_out_of_domain()` 에 쓴 것과 같은 수). 이 자리에
              처음엔 "`test_no_results.py` 가 고정한다"고 적었는데, 그 파일이
              단언하는 것은 문장이 결정적이라는 것과 상수를 안 든다는 것이지
              호출 여부가 아니다 — **검사가 하지 않는 일을 한다고 적으면 그게 곧
              틀린 문서다.** */}
          {data.no_results && (
