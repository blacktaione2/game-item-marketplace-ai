...
**두 한도는 서로 다른 것을 세고, 캐시 적중 취급이 다르다** (ADR-0044).
...
분당까지 적중을 빼면 캐시되는 질의 하나로 서버를 무한정 두드릴 수 있다. 반대로
일일 한도는 비용 계층이라 **LLM 0회짜리 요청이 예산을 깎으면 앞뒤가 안 맞는다** —
화면이 뒤로가기마다 재요청하는 구조라(ADR-0037) 실제로 닿던 경로였다.

**실패한 요청은 소비한다** — 거기까지 갔으면 LLM 호출이 이미 나갔을 수 있고,
실패를 공짜로 만들면 그 자체가 우회로다.
