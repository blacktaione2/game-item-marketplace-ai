...
class Settings(BaseSettings):
    ...
    # v3: `무속성` 추출을 결정적 후처리로 채운다(ADR-0040). **모양이 아니라
    #     내용이 바뀐 경우다** — 같은 질의가 다른 결과를 내놓는다.
    #
    #     ADR-0040 은 "응답 모양은 그대로라 올릴 필요 없다" 고 적었고 **그게
    #     틀렸다.** 위 문단이 *모양*과 *내용* 둘 다라고 말하고 있는데 모양만
    #     읽었다. 배포 직후 `"무속성 검 찾아줘"` 가 옛 응답을 그대로 재생해서
    #     불꽃의 대검이 계속 섞여 나왔다 — 코드는 고쳐졌고 화면만 안 고쳐진,
    #     이 주석이 경고하던 바로 그 증상이다.
    #
    #     **판별법**: 화면의 관측 패널이 `캐시 적중` + `LLM 호출 0회` 를 보이면
    #     그 응답은 배포 전에 저장된 것이다. 코드를 의심하기 전에 배지를 본다.
