...
async def search_items(
    ...
    try:
        ...
        # **나가는 길을 세지 않고 `finally` 에 둔다** (ADR-0046). 성공이든 예외든
        # 소비한다 — 여기까지 왔으면 LLM 호출이 이미 나갔을 수 있고, 공짜로
        # 끝나는 경로를 하나라도 남기면 그게 곧 우회로다.
        #
        # **`_ask_metered` 의 "취소도 소비한다"는 여기 해당하지 않는다.** 그 문장이
        # 가리키는 것은 SSE 라우트의 `task.cancel()` 인데, 이 경로는 평범한
        # 라우트라 클라이언트가 끊어도 Starlette 가 핸들러를 취소하지 않는다.
        # 저장소에 `task.cancel()` 은 `assistant.py` 한 곳뿐이다.
        # *걷지 않는 길을 방어했다고 적으면 그게 곧 틀린 문서다.*
        #
