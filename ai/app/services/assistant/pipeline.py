...
# (그 지시문은 이제 없다. 아래 참고 — **없으면 그 결함이 구조적으로 불가능**하다.)
#
# **합성 코퍼스 고지를 프롬프트에서 뺐다.**
#
# ADR-0038 이 이 문장을 의무화한 이유는 옳았다 — 두 id 공간이 겹쳐서(ADR-0022)
# `23659` 를 자기 거래 번호로 착각할 수 있다. 바뀐 것은 **그 고지를 어디가
# 책임지는가**다.
#
# 화면이 판정 카드 아래에 무조건 적는다 — `거래 합성#23659 · 이 번호는 AI 데모
# 데이터의 id 이며 실제 거래 번호와 다릅니다`. 렌더라서 **모델이 지시를 따르든
# 말든 나온다.** 응답에도 `detection.id_space` 가 구조적으로 실려 간다.
#
# 즉 프롬프트의 그 줄은 **지금은 중복이고, 셋 중 유일하게 실패할 수 있는 층**
# 이었다. 결정 단계가 보장하는 것을 LLM 에게 다시 시키지 않는다는 이 저장소의
# 규칙(ADR-0036 · 0040)에 그대로 걸린다.
#
# **폴백(`_anomaly_answer`)에서도 같이 뺐다.** 한쪽만 빼면 프로바이더가 죽었을
# 때만 문장이 하나 더 붙어, 같은 질의가 상황에 따라 다른 말을 한다.
...
def _anomaly_answer(result: dict[str, Any]) -> str:
    ...
    # 합성 코퍼스 고지는 **여기서 붙이지 않는다.** 화면이 판정 카드에 무조건 적고
    # 응답에는 `id_space` 가 실려 간다 — 자세한 근거는 `_ANOMALY_PROMPT` 위 주석.
    # 프롬프트와 **같이** 뺐다는 점이 중요하다: 한쪽만 빼면 프로바이더가 죽었을
    # 때만 문장이 하나 더 붙는다.
    return sentence
