...
class TestEveryLlmRouteIsMetered:
    ...
    def test_the_dependency_scan_can_actually_fail(self):
        ...
        > 못했다 — *순증가가 손실을 가린다.*
        > `test_no_orphaned_tests.py` 가 이 계열을 저장소 전체에서 막는다.
        >
        > (이 줄은 한 라운드 동안 **존재하지 않는 파일 이름**을 가리키고 있었다 —
        > ADR-0049 가 고친 결함과 같은 모양이 그 ADR 이 만든 파일에서 재발했다.
        > 이제 그 파일이 "없는 테스트 파일을 가리키는 글"도 함께 검사한다.
        > 죽은 이름을 여기 다시 적지 않는 이유가 그것이다 — ADR-0050.)
        """
