...
cd backend && ./gradlew test   # 29건. Postgres·Redis 필요
```

CI가 커밋마다 도는 건 여기까지다 — **AI 118건 + 백엔드 29건 + 프론트 빌드.**
...
| [01-Decisions](docs/01-Decisions/) | **ADR 28건.** 상태/배경/결정/고려한 대안/영향 |
...
| 백엔드 테스트 | 인증·격리 + 도메인 규칙 **29건**. 동시성은 테스트가 아니라 **부하테스트가** 오버셀 0건으로 단언한다 — 단위 테스트로는 재현이 안 되는 영역이다 |
