...
```bash
cd ai && python -m pytest  # 301건. 외부 서비스·모델 불필요
cd backend && ./gradlew test --rerun-tasks   # 57건. Postgres·Redis 필요
```

CI가 커밋마다 도는 건 여기까지다 — **AI 301건 + 백엔드 57건 + 프론트 빌드.**
...
| 백엔드 테스트 | 인증·로그인·격리 + 도메인 규칙 + 알림 흐름 **57건**(실측). 동시성은 테스트가 아니라 **부하테스트가** 오버셀 0건으로 단언한다 — 단위 테스트로는 재현이 안 되는 영역이다 |
