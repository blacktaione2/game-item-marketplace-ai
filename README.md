...
CI가 커밋마다 도는 건 여기까지다 — **AI 118건 + 백엔드 10건 + 프론트 빌드.**
...
| [01-Decisions](docs/01-Decisions/) | **ADR 27건.** 상태/배경/결정/고려한 대안/영향 |
...
| 동기 CPU 호출 | 임베딩·리랭커·KoELECTRA·오토인코더가 전부 `async` 핸들러 안의 동기 호출이라 **이벤트 루프를 막는다**. 캐시 적중 경로만 해결됐고([ADR-0026](docs/01-Decisions/0026-적중-경로-임베딩-지연화.md)), 나머지는 `asyncio.to_thread` 과제로 남았다 — **316 req/s는 그 병목이 남은 상태의 수치**다 |
