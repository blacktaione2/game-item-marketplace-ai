...
cd backend && ./gradlew test --rerun-tasks   # 60건. Postgres·Redis 필요
```
...
| 백엔드 테스트 | 인증·로그인·격리 + 도메인 규칙 + 알림 흐름 **60건**(실측). 동시성은 테스트가 아니라 **부하테스트가** 오버셀 0건으로 단언한다 — 단위 테스트로는 재현이 안 되는 영역이다 |
